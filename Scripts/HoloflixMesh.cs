...
public class HoloflixMesh : MonoBehaviour {
    ...
    */
	...
	private void Awake() {
		if (depthMovieMaterial == null) depthMovieMaterial = GetComponent<Renderer>().sharedMaterial;
	}
	...
	private void Start() {
		...
		if (Application.isPlaying) {
			if (!movie)  {
				...
				if (r) movie = (MovieTexture) r.material.mainTexture;
			}
	...
	private void updateSettings() {
		...
		//this will create too many verts 
		while (meshResX + meshResY > 253) { 
		...
		//we need to account for our own verts here, since our GPU does not support the needed shader
		if (graphicsShaderLevel == 1) vertCount = vertCount * 4; 

		int triangleCount =  meshResX * meshResY * 6;
		if (graphicsShaderLevel == 2) triangleCount = meshResX * meshResY * 3;
		...
		//populate the list.. verts are required in this format due to the way the mesh takes in verts as an argument
	...
	private void checkShaders() {
		...
		//nothing to do here.
		if (currentShader == _lastShader) return;
	...
	private void generateHolovidMesh(GameObject g, Vector2 pos, Vector2 dims, Vector2 UVpos, Vector2 UVdims) {
		...
		if (!meshFilter) meshFilter = g.AddComponent<MeshFilter> ();
